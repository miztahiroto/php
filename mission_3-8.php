...
if(isset($_POST['submit']) && !isset($_POST['henshu'])){
	if(!empty($_POST['comment'])  OR !empty($_POST['image'])){
		$name    = $_POST['name'];
...
//データベースに値を挿入
   		$sql="INSERT INTO asd (name, comment, image, date, pass) VALUES (:name, :comment, :image, :date, :pass)";
   		$stmt = $pdo->prepare($sql);
   		$params = array(':name' => $name, ':comment' => $comment, ':image' => $image, ':date' => $date, ':pass' => $pass);
   		$stmt ->execute($params);
}//img
}//namecomment
}//submit
...
if(isset($_POST['delete'])){
	...
	$delNo = $_POST['deleteNo'];
   	...
   	$sql = $pdo->prepare("SELECT * FROM asd WHERE id =$delNo");
   	$sql ->execute();
...
//DBのpassと削除passが一致したとき
      	if($delPa == $row['pass']){
		$del_sql = "DELETE FROM asd WHERE id = :id";
...
//投稿番号と編集対象番号を比較
   	$sql = $pdo->prepare("SELECT * FROM asd WHERE id =id");
   	$sql ->execute();

//1行取り出し
	while ($row = $sql->fetch(PDO::FETCH_ASSOC)){

//name, comment, imageの再送信
	if(isset($_POST['name']) && isset($_POST['comment'])){
		   $edit_sql = "UPDATE asd SET name = :name, comment = :comment, image = :image, pass = :pass WHERE id = :id";
		   $edit_stmt = $pdo->prepare($edit_sql);
		   $params = array(':name' => $name, ':comment' => $comment, ':image' => $image, ':pass' => $pass,':id' => $ano);
...
<body>
  <h1>プログラミングブログ</h1>
<?php
if(!isset($_POST['edit'])) {//編集ボタンを押さないとき
?>
...
//投稿番号と編集対象番号を比較
   $edit_sql = $pdo->prepare("SELECT * FROM asd WHERE id =$ano");
   $edit_sql ->execute();
...
　   動画・画像<br>
<?php echo "<input type='file' name='image' value='". $image_a ."'><br />";?>
     編集対象番号／パスワード<br>
